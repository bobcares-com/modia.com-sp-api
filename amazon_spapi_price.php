...
// Prepare the feed payload for price update
$feedPayload = [];
foreach ($productCollection as $product) {
    ...
    if (!empty($sku)) {
        ...
        // Retrieve price from the product
        $price = $product->getPrice(); 

        // Check if the price is not empty and greater than 0
        if (!empty($price) && $price > 0) {
            // Store price updates for processing
            $feedPayload[] = [
                'sku' => $sku,
                'price' => $price,
            ];
        } else {
            echo "Price not found or invalid for product with SKU: $sku\n";
        }
    }
}
